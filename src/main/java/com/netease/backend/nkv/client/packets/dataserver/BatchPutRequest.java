...
public class BatchPutRequest extends AbstractRequestPacket {
	...
	public static BatchPutRequest build(short ns, byte[] key, byte[] val, NkvOption opt) throws IllegalArgumentException {
	...
	public static BatchPutRequest build(short ns, Map<ByteArray, byte[]> kv, NkvOption opt) throws IllegalArgumentException {
		BatchPutRequest req = new BatchPutRequest();
		req.setExpired(opt.getExpire());
		req.setVersion(opt.getVersion());
		req.setNamespace(ns);
