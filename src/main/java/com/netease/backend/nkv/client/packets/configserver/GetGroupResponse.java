package com.netease.backend.nkv.client.packets.configserver;
...
public class GetGroupResponse extends AbstractResponsePacket {
	...
	public void decodeFrom(ChannelBuffer in) {
		...
		for (int i = 0; i < count; i++) {
			configs.put(NkvUtil.decodeString(in), 
						NkvUtil.decodeString(in));
		}
		...
		if (count > 0) {
			...
			in.readBytes(temp);
			byte[] result = NkvUtil.deflate(temp);
			...
			while (buff.hasRemaining()) {
				...
				ss.add(NkvUtil.cast2SocketAddress(sid));
				c++;
				if (c == bucketCount) {
					if (valid) {
		...
		count = in.readInt();
		for(int i=0; i<count; i++)
			aliveNodes.add(NkvUtil.cast2SocketAddress(in.readLong()));
