...
public class RangeRequest extends AbstractRequestPacket {
	...
	public static RangeRequest build(short ns, byte[] pkey, byte[] start, byte[] end, int offset, int maxCount, short type, NkvOption opt) throws IllegalArgumentException {
		...
		if (offset < 0 || maxCount < 0) {
			throw new IllegalArgumentException(NkvConstant.OPTION_NOT_AVAILABLE);
		}
